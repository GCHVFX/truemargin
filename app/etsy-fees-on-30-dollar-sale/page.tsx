...
export default function Page() {
  ...
  return (
    <>
      <Script id="tm-etsy-fees-30-jsonld" type="application/ld+json" strategy="afterInteractive" dangerouslySetInnerHTML={{ __html: jsonLd() }} />
      <main className="bg-[#F7F8FA] py-10 sm:py-14">
        <div className="mx-auto max-w-4xl px-4 sm:px-6">
          <article className="rounded-2xl bg-white p-6 text-slate-800 shadow-sm ring-1 ring-slate-200 sm:p-10">
            <h1 className="text-3xl font-semibold tracking-tight text-[#2F3A56] sm:text-4xl">Etsy Fees on a $30 Sale - Full Breakdown</h1>
            <p className="mt-5 text-base leading-7">A $30 sale attracts listing, transaction, and payment processing fees. This breakdown shows how much goes to Etsy and what you retain before product and shipping costs.</p>
            <p className="mt-3 text-base leading-7">At this price point, fee dollars rise quickly with order value. Small price changes can meaningfully affect take-home profit.</p>

            <section className="mt-10 space-y-4">
              <h2 className="text-2xl font-semibold text-[#2F3A56]">Fee breakdown for a $30 order</h2>
              ...
              <p className="leading-7">With Offsite Ads applied, fees can climb to around <strong>${withOffsiteAds.toFixed(2)}</strong>, depending on your region and order specifics.</p>
            </section>

            <section className="mt-10 space-y-4">
              <h2 className="text-2xl font-semibold text-[#2F3A56]">Take-home profit example</h2>
              <p className="leading-7">Before your costs, you keep roughly <strong>${keepBeforeCosts.toFixed(2)}</strong> after baseline Etsy fees. With Offsite Ads, that falls to about <strong>${keepWithOffsite.toFixed(2)}</strong>.</p>
              <p className="leading-7">If cost of goods is $12 and shipping $5, real profit is around $9.70 before ads. That’s why modeling with a calculator helps you price confidently.</p>
            </section>

            <section className="mt-10 space-y-4">
              <h2 className="text-2xl font-semibold text-[#2F3A56]">What affects your profit</h2>
              <p className="leading-7">Fees can apply to shipping charged to the buyer. Regional payment rates differ. Offsite Ads add 12% when they drive the order. Product costs, packaging, and shipping out of pocket further reduce net earnings.</p>
              <p className="leading-7">See how fees compare at other price points: <Link href="/etsy-fees-on-25-dollar-sale" className="font-semibold text-[#2F3A56] underline decoration-[#F4A261] decoration-2 underline-offset-2">Etsy fees on a $25 sale</Link>, <Link href="/etsy-fees-on-40-dollar-sale" className="font-semibold text-[#2F3A56] underline decoration-[#F4A261] decoration-2 underline-offset-2">Etsy fees on a $40 sale</Link>, or <Link href="/etsy-fees-on-50-dollar-sale" className="font-semibold text-[#2F3A56] underline decoration-[#F4A261] decoration-2 underline-offset-2">Etsy fees on a $50 sale</Link>.</p>
            </section>
            ...
            <section className="mt-8 rounded-xl border border-[#F4A261]/35 bg-[#F4A261]/10 p-5 sm:p-6">
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                ...
                <Link href="/etsy-fees-on-100-dollar-sale" className="rounded-md border border-[#2F3A56] bg-white px-4 py-3 text-sm font-semibold text-[#2F3A56] transition hover:bg-slate-50">
                  Etsy Fees on a $100 Sale<span className="mt-1 block text-xs font-normal text-slate-600">Compare fee impact at $100.</span>
                </Link>
              </div>
            </section>
          </article>
