...
export default function Page() {
  ...
  return (
    <>
      <Script id="tm-etsy-fees-15-jsonld" type="application/ld+json" strategy="afterInteractive" dangerouslySetInnerHTML={{ __html: jsonLd() }} />
      <main className="bg-[#F7F8FA] py-10 sm:py-14">
        <div className="mx-auto max-w-4xl px-4 sm:px-6">
          <article className="rounded-2xl bg-white p-6 text-slate-800 shadow-sm ring-1 ring-slate-200 sm:p-10">
            <h1 className="text-3xl font-semibold tracking-tight text-[#2F3A56] sm:text-4xl">Etsy Fees on a $15 Sale (Full Breakdown)</h1>
            <p className="mt-5 text-base leading-7">A $15 sale triggers listing, transaction, and payment processing fees. Here’s a clear breakdown so you know what goes to Etsy and what you keep before product and shipping costs.</p>
            <p className="mt-3 text-base leading-7">On lower-priced items, fixed fees take a bigger slice of the sale, so your margin can feel tighter than the sticker price suggests.</p>

            <section className="mt-10 space-y-4">
              <h2 className="text-2xl font-semibold text-[#2F3A56]">Fee breakdown for a $15 order</h2>
              ...
              <p className="leading-7">If the order came from Offsite Ads, fees can add another 12%, bringing the total closer to <strong>${withOffsiteAds.toFixed(2)}</strong>.</p>
            </section>

            <section className="mt-10 space-y-4">
              <h2 className="text-2xl font-semibold text-[#2F3A56]">Real profit example</h2>
              <p className="leading-7">Before product and shipping costs, you might keep around <strong>${keepBeforeCosts.toFixed(2)}</strong> from a $15 sale after baseline Etsy fees. With Offsite Ads, take-home before costs drops to about <strong>${keepWithOffsite.toFixed(2)}</strong>.</p>
              <p className="leading-7">If your cost of goods is $5 and shipping runs $3, real profit shrinks fast. That’s why many sellers use calculators to model different scenarios.</p>
            </section>

            <section className="mt-10 space-y-4">
              <h2 className="text-2xl font-semibold text-[#2F3A56]">What affects your profit</h2>
              <p className="leading-7">Shipping charged to the buyer can be subject to fees, which changes the math. Your region may have slightly different payment processing rates. Offsite Ads add 12% when they apply. Product costs, packaging, and shipping eat into the remainder.</p>
              <p className="leading-7">For more context on similar price points, see <Link href="/etsy-fees-on-10-dollar-sale" className="font-semibold text-[#2F3A56] underline decoration-[#F4A261] decoration-2 underline-offset-2">Etsy fees on a $10 sale</Link> or <Link href="/etsy-fees-on-25-dollar-sale" className="font-semibold text-[#2F3A56] underline decoration-[#F4A261] decoration-2 underline-offset-2">Etsy fees on a $25 sale</Link>.</p>
            </section>
            ...
            <section className="mt-8 rounded-xl border border-[#F4A261]/35 bg-[#F4A261]/10 p-5 sm:p-6">
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                ...
                <Link href="/etsy-fees-on-50-dollar-sale" className="rounded-md border border-[#2F3A56] bg-white px-4 py-3 text-sm font-semibold text-[#2F3A56] transition hover:bg-slate-50">
                  Etsy Fees on a $50 Sale<span className="mt-1 block text-xs font-normal text-slate-600">Compare fee impact at a higher price point.</span>
                </Link>
              </div>
            </section>
          </article>
