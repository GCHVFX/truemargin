...
export const MARGIN_HEALTH_THRESHOLDS = {
  LOSS: { max: 0.15, label: "Loss", badge: "bg-rose-50 text-rose-700 ring-1 ring-rose-200" },
...
export type MarginHealthTier = "Loss" | "Tight" | "Healthy" | "Strong";

/**
 * Returns margin health tier for a given margin percentage (decimal, 0–1).
 * Thresholds: <15% Loss, 15–<25% Tight, 25–<35% Healthy, >=35% Strong.
 */
export function getMarginHealthTier(marginPct: number): { label: MarginHealthTier; badge: string } {
  ...
  return { label: "Loss", badge: MARGIN_HEALTH_THRESHOLDS.LOSS.badge };
}
